...
enum ioType { REG, RAM_ADDR, RAM, IMMED };

enum Flag { HELP, COMPILE, COMPILE_ALL, FROM_BIN, MAKE_ARRAY };
...
void writeBytecodeFile(const std::string &path,
                       const std::vector<byte_t> &bytecode);
void writeDebugcodeFile(const std::string &path,
                        const std::vector<byte_t> &debugcode);
void writeBytecodeArrayFiles(const std::string &path,
                             const std::vector<byte_t> &bytecode);
...
int main(int argc, const char *argv[]) {
  ...
  if (flags.find(HELP) != std::end(flags)) {
    std::cout << "[4BP_ASM]" << std::endl
              ...
              << "-b run program from byte code. program code should be byte "
                 "code and optionally debug byte code"
              << std::endl;
    return 0;
  }
...
std::vector<byte_t> readBytecodeFile(const std::string &path) {
  ...
  auto bytecode = std::vector<byte_t>((std::istreambuf_iterator<char>(fin)),
...
std::vector<byte_t> readDebugcodeFile(const std::string &path) {
  ...
  auto debugcode = std::vector<byte_t>((std::istreambuf_iterator<char>(fin)),
                                       (std::istreambuf_iterator<char>()));
  fin.close();
...
parseProgram(const std::string &path) {
  ...
  for (size_t addr = 0; addr < cmds.size(); ++addr) {
    ...
    lexems.erase(std::remove_if(std::begin(lexems), std::end(lexems),
                                [](auto &x) { return x == ""; }),
                 std::end(lexems));
...
void writeBytecodeArrayFiles(const std::string &path,
  ...
  for (auto i = 0; i < bytecode.size(); i += 3) {
    byte_t op = bytecode[i], c1 = bytecode[i + 1], c2 = bytecode[i + 2];
...
void execProgram(const std::vector<byte_t> &bytecode,
  ...
  for (ssize_t itr = 0; itr < execcode.size() || itr == execcode.size() - 1;
       itr += 4) {
    ...
    std::cout << "\nreg: ";
    for (auto i = 0; i < 16; ++i)
      std::cout << static_cast<int>(regs[i]) << " ";
    std::cout << "\n------------------------------------\n";
