...
class IC74273(Component):
    ...
    def propagate(self):
        ...
        if clk and not self.prev_clk:
            new_val = 0
            if self.get(self.D1): new_val |= 1
            if self.get(self.D2): new_val |= 2
            if self.get(self.D3): new_val |= 4
            if self.get(self.D4): new_val |= 8
            if self.get(self.D5): new_val |= 16
            if self.get(self.D6): new_val |= 32
            if self.get(self.D7): new_val |= 64
            if self.get(self.D8): new_val |= 128
            self.state = new_val
            self._update_outputs()
