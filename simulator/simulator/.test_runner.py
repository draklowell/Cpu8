...
from base import Component, Network
...
def test_ic7400_basic():
    pins = create_network_dict(
        [IC7400.VCC, IC7400.GND, IC7400.A1, IC7400.B1, IC7400.Y1]
    )
    ic = IC7400("U1", pins)
...
def test_ic7402_basic():
    pins = create_network_dict(
        [IC7402.VCC, IC7402.GND, IC7402.A1, IC7402.B1, IC7402.Y1]
    )
    ic = IC7402("U2", pins)
...
def test_ic74138_decoding():
    ...
    for i in range(8):
        ...
        for j, out_pin in enumerate(outputs):
            if j == i:
                tf.assert_false(
                    pins[out_pin].get(), f"Y{i} повинен бути LOW при адресі {i}"
                )
            else:
                tf.assert_true(
                    pins[out_pin].get(), f"Y{j} повинен бути HIGH при адресі {i}"
                )
...
def test_ic74154_decoding():
    ...
    for i in range(4):
        ...
        tf.assert_true(
            pins[output_pins[(i + 1) % 16]].get(), f"Y{(i+1) % 16} повинен бути HIGH"
        )
...
def test_ic74245_output_enable():
    pins = create_network_dict(
        [IC74245.VCC, IC74245.GND, IC74245.OE, IC74245.DIR, IC74245.A1, IC74245.B1]
    )
    ic = IC74245("U9", pins)
...
def test_ic74574_output_enable():
    pins = create_network_dict(
        [IC74574.VCC, IC74574.GND, IC74574.CLK, IC74574.OE, IC74574.D1, IC74574.Q1]
    )
    ic = IC74574("U11", pins)
...
def test_eeprom_read():
    ...
    pins = create_network_dict(
        [EEPROM.VCC, EEPROM.GND, EEPROM.CE, EEPROM.OE, EEPROM.WE] + addr_pins + io_pins
    )
