...
const obj::SectionDescription* getSection(const obj::ObjectFile& object,
                                          std::size_t index) {
...
link::ResolvedSym resolveDefinedSymbol(const obj::ObjectFile& object,
    ...
    if (const uint32_t logical_size = sectionLogicalSize(object, symbol.section_index);
        value > logical_size) {
        throw std::runtime_error("Symbol '" + symbol.name + "' offset 0x" +
                                 toUpperHex(value) + " exceeds section " +
                                 sectionNameFromIndex(symbol.section_index) +
                                 " size 0x" + toUpperHex(logical_size));
    }

    uint64_t base = 0;
    switch (symbol.section_index) {
    case 0: {
        base = static_cast<uint64_t>(plan.layout.text_base) +
               plan.text_offsets[object_index];
        break;
    }
    case 2: {
        base = static_cast<uint64_t>(plan.layout.bss_base) +
               plan.bss_offsets[object_index];
        break;
    }
    case 3: {
        base = static_cast<uint64_t>(plan.layout.rodata_base) +
               plan.rodata_offsets[object_index];
        break;
    }
    case 1:
        throw std::runtime_error(
            "Initialized .data section is not supported for symbol '" + symbol.name +
            "'");
    default:
        throw std::runtime_error("Symbol '" + symbol.name +
                                 "' located in unsupported section");
    }
    ...
    return link::ResolvedSym{symbol.section_index, static_cast<uint32_t>(absolute),
                             symbol.bind};
}

link::ResolvedSym
resolveSymbolForReloc(const obj::ObjectFile& object, std::size_t object_index,
                      const obj::SymbolDescription& symbol, const link::MergePlan& plan,
...
RelocResolver::buildGlobalSymtab(const std::vector<obj::ObjectFile>& objects,
    ...
    for (std::size_t obj_index = 0; obj_index < objects.size(); ++obj_index) {
        ...
        for (const auto& symbol : object.symbols) {
            if (symbol.section_index >= 0) {
                const auto resolved =
                    resolveDefinedSymbol(object, obj_index, symbol, plan);
                ...
                if (it != table.end()) {
                    if (it->second.section_index >= 0) {
                        throw std::runtime_error("Multiple definition of symbol '" +
                                                 symbol.name + "'");
                    }
                    it->second = resolved;
                } else {
                ...
                auto [it, inserted] =
                    table.emplace(symbol.name, ResolvedSym{-1, 0, symbol.bind});
...
void RelocResolver::apply(const std::vector<obj::ObjectFile>& objects,
    ...
    for (std::size_t obj_index = 0; obj_index < objects.size(); ++obj_index) {
        ...
        for (const auto& reloc : object.reloc_entries) {
            ...
            const auto resolved =
                resolveSymbolForReloc(object, obj_index, symbol, plan, gsym);

            if (reloc.type != obj::RelocType::ABS16) {
                throw std::runtime_error("Unsupported relocation type for symbol '" +
                                         symbol.name + "'");
            }
            ...
            const int64_t relocated_value =
                static_cast<int64_t>(resolved.abs_addr) + reloc.addend;
            if (relocated_value < 0 ||
                relocated_value > std::numeric_limits<uint16_t>::max()) {
                throw std::runtime_error("Relocation result out of range for symbol '" +
                                         symbol.name + "'");
            }
