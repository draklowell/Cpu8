...
namespace {
std::vector<OT> Sig(std::initializer_list<OT> v) { return {v}; }
} // namespace
...
EncodeTable::EncodeTable() {
    auto add_simple = [&](const char* mnemonic, std::vector<OT> signature,
                          const uint8_t opcode, const uint8_t size,
                          const bool reloc = false, const uint8_t imm_offset = 1) {
        Key key{std::string(mnemonic), signature};
        table.emplace(std::move(key),
                      OpcodeSpecs{opcode, size, signature, reloc, imm_offset});
    };
...
uint8_t EncodeTable::movOpcode(Reg dst, Reg src) const {
    return mov_[static_cast<int>(dst)][static_cast<int>(src)];
}
...
const std::unordered_map<Key, OpcodeSpecs, KeyHash>& EncodeTable::entries() const {
    return table;
}
} // namespace asmx
