 ...
 * This enum class defines the types of operands that can be used in assembly
 * instructions.
 */
 ...
 * @note The `signature` field is a vector of operand types that define the
 * instruction's operands.
 * @note The `needs_reloc` field indicates whether the instruction requires relocation.
 * @note The `imm_offset` field specifies the offset for immediate values within the
 * instruction.
 */
struct OpcodeSpecs {
    uint8_t                  opcode{0};
 ...
 *
 * @note The `mnemonic` field represents the instruction's mnemonic (e.g., "MOV",
 * "ADD").
 * @note The `signature` field is a vector of operand types that define the
 * instruction's operands.
 * @note The equality operator (`operator==`) is overridden to compare two keys based on
 ...
 * This struct provides a hash function for the Key structure, enabling its use in
 * unordered containers. The hash is computed based on the mnemonic and operand
 * signature.
 * @note The hash function combines the hash of the mnemonic with the hashes of each
 * operand type in the signature.
 */
struct KeyHash {
 ...
 * It also provides methods to retrieve specific opcodes for common instructions like
 * MOV and LDI.
 *
 * @note The class is implemented as a singleton, with a private constructor and a
 * static `get` method.
 * @note The `find` method allows for searching the table for a specific instruction
 * encoding.
 * @note The class includes precomputed opcode tables for certain instructions to
...
class EncodeTable {
    ...
    uint8_t ldi8_[static_cast<int>(Reg::Invalid)]{},
        ldi16_[static_cast<int>(Reg::Invalid)]{};
    uint8_t ld16_[static_cast<int>(Reg::Invalid)]{},
        st16_[static_cast<int>(Reg::Invalid)]{};
