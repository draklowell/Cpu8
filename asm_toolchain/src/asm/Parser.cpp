...
namespace asmx
{
    namespace
    {
        [[nodiscard]] bool isIdentStart(const char ch)
        {
            return std::isalpha(static_cast<unsigned char>(ch)) != 0 || ch == '_';
        }

        [[nodiscard]] bool isIdentChar(const char ch)
        {
            return std::isalnum(static_cast<unsigned char>(ch)) != 0 || ch == '_' || ch == '.';
        }

        [[nodiscard]] util::SourceLoc makeLoc(const std::string &file, const uint32_t line, const uint32_t col)
        {
        ...
        [[nodiscard]] uint32_t parseNumber(const Token &token)
        {
            ...
            else if (text_size >= 2 && (text[0] == '0') && (text[1] == 'b' || text[1] == 'B'))
            {
                if (text_size == 2)
                {
    ...
    std::vector<Token> Parser::lex(const std::string &text, const std::string &file)
    {
        ...
        uint32_t line = 1;
        uint32_t col = 1;
        const auto pushToken = [&](const TokenKind kind, std::string tokenText, const uint32_t tokenLine, const uint32_t tokenCol)
        {
            tokens.push_back(Token{kind, std::move(tokenText), makeLoc(file, tokenLine, tokenCol)});
        };
        ...
        while (i < text.size())
        {
            ...
            if (ch == '\r')
            {
                ...
                ++i;
                pushToken(TokenKind::NewLine, "", line, locCol);
                ++line;
                col = 1;
                continue;
                ;
            ...
            if (std::isspace(ch) || ch == '\t' || ch == '\v' || ch == '\f')
            {
                ++i;
                ++col;
                continue;
            ...
            if (ch == '[')
            {
                pushToken(TokenKind::LBracket, "[", line, col);
                ++i;
                ++col;
                continue;
            }
            ...
            if (ch == ']')
            {
                pushToken(TokenKind::RBracket, "]", line, col);
                ++i;
                ++col;
                continue;
            }
            if (ch == ',')
            {
                pushToken(TokenKind::Comma, ",", line, col);
                ++i;
                ++col;
                continue;
            }
            if (ch == ':')
            {
                pushToken(TokenKind::Colon, ":", line, col);
                ++i;
                ++col;
                continue;
            }
            if (ch == '.')
            {
                pushToken(TokenKind::Dot, ".", line, col);
                ++i;
                ++col;
                continue;
            }
            if (isIdentStart(ch))
            {
                ...
                pushToken(TokenKind::Ident, text.substr(start, i - start), line, startCol);
                continue;
            }
            if (std::isdigit(static_cast<unsigned char>(ch)) != 0)
            {
                ...
                pushToken(TokenKind::Number, text.substr(start, i - start), line, startCol);
                continue;
            }
            throw util::Error(makeLoc(file, line, col), "Unexpected character in input");
        }
        pushToken(TokenKind::Eof, "", line, col);
        return tokens;
    }
    ...
    ParseResult Parser::parse(const std::vector<Token> &tokens)
    {
        ...
        while (index < tokens.size())
        {
            ...
            while (index < tokens.size() && tokens[index].kind != TokenKind::NewLine && tokens[index].kind != TokenKind::Eof)
            {
            ...
            if (first.kind == TokenKind::Ident)
            {
                if (lineTokens.size() >= 2 && lineTokens[1].kind == TokenKind::Colon)
                {
                    ...
                    {
                        throw util::Error(lineTokens[2].loc, "Unexpected tokens after label definition");
                    }
                    Label label;
                ...
                while (argIndex < lineTokens.size())
                {
                    ...
                    if (current.kind == TokenKind::Comma)
                    {
                        if (!needComma)
                        {
                            throw util::Error(current.loc, "Unexpected comma in argument list");
                        }
                        needComma = false;
                        ++argIndex;
                        continue;
            ...
            if (first.kind == TokenKind::Dot)
            {
                ...
                while (argIndex < lineTokens.size())
                {
                    ...
                    if (cur.kind == TokenKind::Comma)
                    {
                        if (!expectValue)
                        {
                            throw util::Error(cur.loc, "Unexpected comma in directive arguments");
                        }
                        expectValue = false;
                        ++argIndex;
                        continue;
                    ...
                    {
                        throw util::Error(cur.loc, "Unexpected token in directive arguments");
                    }
                    ++argIndex;
                    expectValue = true;
                }
